...
***** defhydra
#+begin_src emacs-lisp
(defhydra hydra-frame-menu (:color blue)
  "
 ^Action^
^^^^^^^^^--------------------------------------------------------
...
#+end_src
*** org-contrib
Tangle this if you need the additional packages that bundled in =org-contrib=. Check the [[https://orgmode.org/worg/org-contrib/][website]] for details of packages it contains.
#+begin_src emacs-lisp :tangle no
...
#+begin_src emacs-lisp
  ...
  (customize-set-variable  'org-num-face '(:inherit org-special-keyword :underline nil :weight bold))
  (customize-set-variable 'org-num-skip-tags '("noexport" "nonum"))
  ...
  ;;program to send notification with
...
#+end_src
**** startups
This should be set per file
#+begin_src emacs-lisp
(setq org-startup-indented t ; turn on `org-indent-mode' at startup
      org-startup-with-inline-images t ; show inline images
      org-startup-align-all-tables t ; align all tables when visiting a file
      org-startup-folded nil ; no folding
      )
#+end_src
**** pretty entities
