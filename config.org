...
***** hint
      The next step is create a [[https://github.com/abo-abo/hydra/wiki/Conditional-Hydra][conditional hydra]], this is also why in the previous [[*defhydra][defhydra]], the =:hint= is set to nil, so this hint will show depending on the value of =one-window-p=.
      #+begin_src emacs-lisp
      ...
      " hydra-window-menu/heads))
                   ...
                   (define-key hydra-window-menu/keymap "[" nil)
                   (define-key hydra-window-menu/keymap "]" nil)
                   (define-key hydra-window-menu/keymap "q" 'hydra-window-menu/keyboard-quit))
               (prog1 (eval
                       (hydra--format nil '(nil nil :hint nil)
                                      "
      ^Split^                    ^Action^                                 ^Resize^
