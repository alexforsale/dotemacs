...
#+begin_src emacs-lisp
  ...
  ;; (require 'config-dockerfile-mode)
#+end_src
** Org packages
#+begin_src emacs-lisp
...
** builtins
:PROPERTIES:
:header-args: :tangle (expand-file-name "modules/builtin-ui.el" user-emacs-directory) :mkdirp t
:END:
*** header
#+begin_src emacs-lisp
...
#+end_src
*** disable lockfiles
While it's better to have one, I have a bad habit of shutting down PC/Laptops with the power buttons, and the confirmation gets annoying.
#+begin_src emacs-lisp
...
** csh-mode
:PROPERTIES:
:header-args: :tangle (expand-file-name "site-lisp/csh-mode.el" user-emacs-directory) :mkdirp t
:END:
*** header
#+begin_src emacs-lisp
...
** dockerfile-mode
:PROPERTIES:
:header-args: :tangle (expand-file-name "modules/config-dockerfile-mode.el" user-emacs-directory)
:END:
*** header
#+begin_src emacs-lisp
  ;;; config-dockerfile-mode.el --- Summary -*- lexical-binding: t -*-
  ;;; Commentary:
  ;; This file is auto-generated from `config.org'
  ;; See `https://java281.dynv6.net/~alexforsale/posts/Emacs-Configuration.html'
  ;;; Code:
...
#+begin_src emacs-lisp
  ;; alows single character alphabetical bullets
  (customize-set-variable 'org-list-allow-alphabetical t)
#+end_src
**** save target buffer after archiving nodes
#+begin_src emacs-lisp
...
#+end_src
*** package
Use built-in package when in guix
#+begin_src emacs-lisp
  ;; use built-in when in guix
  (if (or (string= "guix" (getenv "DESKTOP_SESSION"))
          (getenv "GUIX_LOCPATH"))
      (straight-use-package '(magit :type built-in))
...
** no-littering
:PROPERTIES:
:header-args: :tangle (expand-file-name "modules/config-no-littering.el" user-emacs-directory)
:END:
Keeps our =~/.emacs.d= clean
*** header
#+begin_src emacs-lisp
  ;;; config-no-littering.el --- Summary -*- lexical-binding: t -*-
  ;;; Commentary:
  ;;; Code:
#+end_src
*** package
...
*** footer
#+begin_src emacs-lisp
  (provide 'config-no-littering)
  ;;; config-no-littering.el ends here
#+end_src
* email
** builtins
:PROPERTIES:
